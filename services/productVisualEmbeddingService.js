...
async function prepareImageBufferForNvClip(buffer) {
    ...
    for (let i = 1; i < steps.length && out.length > NV_CLIP_MAX_IMAGE_BYTES; i += 1) {
...
async function embedImageBuffer(imageBuffer, options = {}) {
    ...
    const { buffer: prepared, mimeExt } = await prepareImageBufferForNvClip(imageBuffer);
    ...
    while (attempt < maxAttempts) {
        ...
        if (!response.ok) {
            ...
            const detail =
                ...
                json?.message ||
                json?.detail ||
                (json && typeof json === 'object' ? JSON.stringify(json).slice(0, 900) : null) ||
                rawBody?.slice(0, 900) ||
                '(empty body)';
...
async function syncPrimaryProductImageEmbedding(productId) {
    ...
    if (existing?.content_hash === contentHash && existing?.embedding_model === model) {
...
async function searchSimilarProductsByImageUrl(imageUrl, limit = 12) {
    ...
    const active = products.filter((p) => p && p.is_active !== false && p.is_active !== 0);
...
async function syncAllProductImageEmbeddings(options = {}) {
    ...
    const delayMs = Number.parseInt(process.env.PRODUCT_VISUAL_SYNC_DELAY_MS, 10) || 400;
    const logEvery = Math.max(1, Number.parseInt(process.env.PRODUCT_VISUAL_SYNC_LOG_EVERY, 10) || 20);

    const [rows] = await pool.execute(
        'SELECT id FROM products WHERE is_active = TRUE ORDER BY id ASC'
    );

    console.log(`[visual-sync] ${rows.length} active products (log every ${logEvery} rows; delay ${delayMs}ms)`);
